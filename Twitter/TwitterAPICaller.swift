...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    static let client = TwitterAPICaller(baseURL: URL(string: "https://api.twitter.com"), consumerKey: "erdzHUUycl3IcfN2fOYyxTaW6", consumerSecret: "34w3RtHuN8wrrAzhNCzBNESZ0rVGv9TUvrSXjKETTCOSOANArE")
    var loginSuccess: (() -> ())?
    ...
    func login(url: String, success: @escaping () -> (), failure: @escaping (Error) -> ()){
        ...
        TwitterAPICaller.client?.fetchRequestToken(withPath: url, method: "GET", callbackURL: URL(string: "alamoTwitter://oauth"), scope: nil, success: { (requestToken: BDBOAuth1Credential!) -> Void in
            let url = URL(string: "https://api.twitter.com/oauth/authorize?oauth_token=\(g")!
